...
class Accuracy(Metric):
    ...
    def __init__(self, pos_tag_pad_index: int):
        self.num_correct = 0
        self.total_predictions = 0
    ...
    def update(self, logits: torch.FloatTensor, pos_tags: torch.LongTensor) \
            -> float:
        """
        ...
        """
        batch_num_correct = int(((logits.argmax(axis=-1) == pos_tags) &
                                 (pos_tags != self.pos_tag_pad_index)).sum())
        batch_num_predictions = pos_tags.numel() - \
                                int((pos_tags == self.pos_tag_pad_index).sum())
